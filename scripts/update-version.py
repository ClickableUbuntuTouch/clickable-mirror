...
with open(docs_conf_file, 'r') as fr:
    ...
    for line in docs_conf:
        if line.startswith('version ='):
            updated_docs_conf.append(f'version = \'{new_version}\'\n')
        elif line.startswith('release ='):
            updated_docs_conf.append(f'release = \'{new_version}\'\n')
        else:
...
with open(version_file, 'r') as fr:
    ...
    for line in version_data:
        if version_re.match(line):
            updated_version_data.append(f'__version__ = \'{new_version}\'\n')
        else:
...
with open(deb_changelog_file, 'r') as fr:
    ...
    deb_changelog = [f'clickable ({new_version}) unstable; urgency=medium\n\n\
  * TODO\n\n\
...
with open(docs_changelog_file, 'r') as fr:
    ...
    docs_changelog.insert(5, f'Changes in v{new_version}\n-----------------\n\n- TODO\n\n')
